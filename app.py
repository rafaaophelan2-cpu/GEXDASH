...
def clean_ai_response(text: str) -> str:
    """
    Elimina bloques de código Markdown innecesarios, delimitadores LaTeX y arregla el formateo.
    """
    if not text:
        return ""
    
    # 1. Eliminar bloques/fences de código markdown tipo ```html ... ``` o ``` ... ```
    cleaned = re.sub(r'```(?:html|markdown)?', '', text)
    cleaned = cleaned.replace('```', '')

    # 2. Eliminar símbolos de dólar que fuerzan LaTeX en Streamlit
    cleaned = cleaned.replace("$", "")
    
    # 3. Eliminar delimitadores de fórmulas matemáticas LaTeX
    cleaned = cleaned.replace(r"\(", "").replace(r"\)", "").replace(r"\[", "").replace(r"\]", "")
    
    # 4. Eliminar asteriscos o guiones bajos de itálicas sueltos
...
def consultar_ia(tipo_analisis="Análisis General", mensaje_usuario=None):
    ...
    2. ESTRICTAMENTE PROHIBIDO RESPONDER CON TEXTO PLANO O LISTAS TRIVIALES.
    3. DEBES ESTRUCTURAR LA RESPUESTA EN 4 SECCIONES CLARAS:
    ...
    4. EN LA HOJA DE RUTA DEBES EXPLICAR ESCENARIOS CONDICIONALES DETALLADOS (IF-THEN):
       - "Si rompemos CW1 (X USD) y consolidamos -> Acelerará hacia CW2 (Y USD) y CW3 (Z USD)... Si fallamos en romper CW1 -> Rechazo directo buscando Zero Gamma (W USD)..."
       - "Si perdemos Zero Gamma (W USD) y quiebra PW1 (A USD) -> Venta forzada de dealers hacia PW2 (B USD)... Si PW1 sostiene el impacto -> Rebote táctico a..."
    """
...
with st.sidebar.popover("💬 ASISTENTE IA GEX", use_container_width=True):
    ...
    for msg in st.session_state.chat_messages:
        with st.chat_message(msg["role"]):
            ...
            if msg["role"] == "assistant" and ("<div" in content_str or "<p" in content_str or content_str.startswith("<")):
    ...
    if chat_input := st.chat_input("Pregunta sobre GEX, Griegas o escenarios condicionales..."):
        ...
        with st.spinner("Analizando escenarios de mercado..."):
            ...
            with st.chat_message("assistant"):
                content_str = str(respuesta_bot).strip()
                if "<div" in content_str or "<p" in content_str or content_str.startswith("<"):
                    st.markdown(content_str, unsafe_allow_html=True)
                else:
...
with tab_greeks:
    st.markdown('<div class="depth-frame">', unsafe_allow_html=True)
    st.markdown("<h3 style='margin-top:0; font-weight:800; color:#F0F6FC; font-size:1.1rem; letter-spacing:0.5px;'>📊 PERFILES DE EXPOSICIÓN A GRIEGAS SECUNDARIAS</h3>", unsafe_allow_html=True)
    
    if not df_curr.empty and 'strike' in df_curr.columns:
        df_sub = df_curr[(df_curr['strike'] >= min_strike) & (df_curr['strike'] <= max_strike)].copy()
        if not df_sub.empty:
            tab_g1, tab_g2, tab_g3 = st.tabs(["DELTA (DEX)", "CHARM (CHEX)", "VANNA (VANNA)"])
            xaxis_kwargs = safe_strike_range(df_sub)

            with tab_g1:
                fig_dex = go.Figure()
                fig_dex.add_trace(go.Bar(x=df_sub['strike'], y=df_sub['net_dex'], marker_color=['#10B981' if v>=0 else '#EF4444' for v in df_sub['net_dex']]))
                if spot_price > 0: fig_dex.add_vline(x=spot_price, line_color="#3B82F6", line_dash="dash")
                fig_dex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="Net Delta Exposure ($M / +1% Spot)", height=480, xaxis=xaxis_kwargs)
                st.plotly_chart(fig_dex, use_container_width=True)

            with tab_g2:
                fig_chex = go.Figure()
                fig_chex.add_trace(go.Bar(x=df_sub['strike'], y=df_sub['net_chex'], marker_color=['#10B981' if v>=0 else '#EF4444' for v in df_sub['net_chex']]))
                if spot_price > 0: fig_chex.add_vline(x=spot_price, line_color="#3B82F6", line_dash="dash")
                fig_chex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="Net Charm Exposure ($M Delta / día por tiempo)", height=480, xaxis=xaxis_kwargs)
                st.plotly_chart(fig_chex, use_container_width=True)

            with tab_g3:
                fig_vanna = go.Figure()
                fig_vanna.add_trace(go.Bar(x=df_sub['strike'], y=df_sub['net_vanna'], marker_color=['#10B981' if v>=0 else '#EF4444' for v in df_sub['net_vanna']]))
                if spot_price > 0: fig_vanna.add_vline(x=spot_price, line_color="#3B82F6", line_dash="dash")
                fig_vanna.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="Net Vanna Exposure ($M Delta / +1% Volatilidad)", height=480, xaxis=xaxis_kwargs)
...
with tab_back:
    st.markdown('<div class="depth-frame">', unsafe_allow_html=True)
    st.markdown("<h3 style='margin-top:0; font-weight:800; color:#F0F6FC; font-size:1.1rem;'>📜 Histórico e Intradía Backgamma</h3>", unsafe_allow_html=True)
    
    if jsonbin_history_data:
        available_dates = sorted(list(jsonbin_history_data.keys()), reverse=True)
        selected_date = st.selectbox("Seleccionar Fecha Guardada:", available_dates)
        
        snaps = jsonbin_history_data.get(selected_date, [])
        if snaps:
            st.info(f"Mostrando {len(snaps)} capturas para la fecha {selected_date}")
            times_b = [s.get("time") for s in snaps]
            spots_b = [s.get("spot") for s in snaps]
            gex_b = [s.get("net_gex") for s in snaps]

            fig_back = make_subplots(specs=[[{"secondary_y": True}]])
            fig_back.add_trace(go.Scatter(x=times_b, y=gex_b, name="Net GEX", line=dict(color="#F59E0B", width=2)), secondary_y=False)
            fig_back.add_trace(go.Scatter(x=times_b, y=spots_b, name="Spot Price", line=dict(color="#3B82F6", width=2)), secondary_y=True)
            fig_back.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', height=500)
            st.plotly_chart(fig_back, use_container_width=True)
    else:
        st.warning("No hay registros históricos disponibles en la nube.")
...
with tab_data:
    st.markdown('<div class="depth-frame">', unsafe_allow_html=True)
    st.markdown("<h3 style='margin-top:0; font-weight:800; color:#F0F6FC; font-size:1.1rem;'>🗄️ TABLA DE DATOS TÉCNICOS DETALLADA</h3>", unsafe_allow_html=True)
    
    if not df_curr.empty:
        df_disp = df_curr.copy()
        cols_show = ['strike', 'openInterest_c', 'openInterest_p', 'call_gex', 'put_gex', 'net_gex', 'net_dex', 'net_chex', 'net_vanna']
        cols_exist = [c for c in cols_show if c in df_disp.columns]
        st.dataframe(df_disp[cols_exist], use_container_width=True, height=500)
    else:
