...
with tab_greeks:
    ...
    # Métricas neutrales con dinámicos Verde (Positivo) y Rojo (Negativo)
    ...
    g2.markdown(f'<div class="metric-card"><div class="metric-label">Net Theta (TEX)</div><div class="metric-value" style="color:{c_tex};">${net_tex_total:,.0f}</div><div class="metric-sub">Decaimiento / Día</div></div>', unsafe_allow_html=True)
    g3.markdown(f'<div class="metric-card"><div class="metric-label">Net Vega (VEX)</div><div class="metric-value" style="color:{c_vex};">${net_vex_total:,.0f}</div><div class="metric-sub">Por +1% IV</div></div>', unsafe_allow_html=True)
    ...
    if not df_curr.empty and 'strike' in df_curr.columns:
        ...
        with sub_grk1:
            if not df_grk_sub.empty and 'net_dex' in df_grk_sub.columns:
                ...
                fig_net_dex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Net Delta Exposure (DEX) Profile por Strike (M USD)</b>", xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
                st.plotly_chart(fig_net_dex, use_container_width=True)

                st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)

                # Calls vs Puts (Paleta Única Delta: Cyan Claro #38BDF8 / Azul Oscuro #1E40AF)
                fig_dex = go.Figure()
                ...
                fig_dex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Call vs Put Delta Exposure (DEX) por Strike (M USD)</b>", barmode='relative', xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
        ...
        with sub_grk2:
            if not df_grk_sub.empty and 'net_tex' in df_grk_sub.columns:
                ...
                fig_net_tex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Net Theta Exposure (TEX) Profile por Strike (USD/Día)</b>", xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
                st.plotly_chart(fig_net_tex, use_container_width=True)

                st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)

                # Calls vs Puts (Paleta Única Theta: Ámbar Claro #FBBF24 / Marrón Oscuro #78350F)
                fig_tex = go.Figure()
                fig_tex.add_trace(go.Bar(x=df_grk_sub['strike'], y=df_grk_sub['call_tex'], name="Call TEX (+)", marker_color='#FBBF24'))
                fig_tex.add_trace(go.Bar(x=df_grk_sub['strike'], y=df_grk_sub['put_tex'], name="Put TEX (-)", marker_color='#78350F'))
                if spot_price > 0: fig_tex.add_vline(x=spot_price, line_color="#3B82F6", line_dash="dash", annotation_text=f"Spot (${spot_price:.2f})")
                fig_tex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Call vs Put Theta Exposure (TEX) por Strike</b>", barmode='relative', xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
        ...
        with sub_grk3:
            if not df_grk_sub.empty and 'net_vex' in df_grk_sub.columns:
                ...
                fig_net_vex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Net Vega Exposure (VEX) Profile por Strike (USD / +1% IV)</b>", xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
                st.plotly_chart(fig_net_vex, use_container_width=True)

                st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)

                # Calls vs Puts (Paleta Única Vega: Turquesa Claro #2DD4BF / Verde Teal Oscuro #115E59)
                fig_vex = go.Figure()
                fig_vex.add_trace(go.Bar(x=df_grk_sub['strike'], y=df_grk_sub['call_vex'], name="Call VEX (+)", marker_color='#2DD4BF'))
                fig_vex.add_trace(go.Bar(x=df_grk_sub['strike'], y=df_grk_sub['put_vex'], name="Put VEX (-)", marker_color='#115E59'))
                if spot_price > 0: fig_vex.add_vline(x=spot_price, line_color="#3B82F6", line_dash="dash", annotation_text=f"Spot (${spot_price:.2f})")
                fig_vex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Call vs Put Vega Exposure (VEX) por Strike</b>", barmode='relative', xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
        ...
        with sub_grk4:
            if not df_grk_sub.empty and 'net_chex' in df_grk_sub.columns:
                ...
                fig_net_chex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Net Charm Exposure (CHEX) Profile por Strike (M USD/Día)</b>", xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
                st.plotly_chart(fig_net_chex, use_container_width=True)

                st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)

                # Calls vs Puts (Paleta Única Charm: Naranja Claro #FB923C / Óxido Oscuro #9A3412)
                fig_chex = go.Figure()
                fig_chex.add_trace(go.Bar(x=df_grk_sub['strike'], y=df_grk_sub['call_chex'], name="Call CHEX (+)", marker_color='#FB923C'))
                fig_chex.add_trace(go.Bar(x=df_grk_sub['strike'], y=df_grk_sub['put_chex'], name="Put CHEX (-)", marker_color='#9A3412'))
                if spot_price > 0: fig_chex.add_vline(x=spot_price, line_color="#3B82F6", line_dash="dash", annotation_text=f"Spot (${spot_price:.2f})")
                fig_chex.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Call vs Put Charm Exposure (CHEX) por Strike</b>", barmode='relative', xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
        ...
        with sub_grk5:
            if not df_grk_sub.empty and 'net_vanna' in df_grk_sub.columns:
                ...
                fig_net_vanna.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Net Vanna Exposure (VANNA) Profile por Strike (M USD)</b>", xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
                st.plotly_chart(fig_net_vanna, use_container_width=True)

                st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)

                # Calls vs Puts (Paleta Única Vanna: Rosado Claro #F472B6 / Morado Oscuro #6B21A8)
                fig_vanna = go.Figure()
                ...
                fig_vanna.update_layout(template="plotly_dark", plot_bgcolor='#06080D', paper_bgcolor='#06080D', title="<b>Call vs Put Vanna Exposure (VANNA) por Strike</b>", barmode='relative', xaxis=dict(title="Strike ($)", **xaxis_kwargs_grk), height=420)
...
with tab_back:
    ...
    if hist_snaps:
        ...
        if dates_avail:
            ...
            st.info("No se encontraron fechas registradas en JSONBin.")
    else:
        st.info("No se encontró historial en Supabase ni JSONBin.")
    st.markdown('</div>', unsafe_allow_html=True)

# --- 6. DATA ---
with tab_data:
    st.markdown('<div class="depth-frame">', unsafe_allow_html=True)
    st.markdown("<h3 style='margin-top:0; font-weight:800; color:#F0F6FC; font-size:1.1rem;'>📁 TABLA COMPLETA DE DATOS (CADENA DE OPCIONES)</h3>", unsafe_allow_html=True)
